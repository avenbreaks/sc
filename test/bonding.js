...
describe("Bonding Curve Tests", function () {
  ...
  before(async function () {
    ...
    let numTokens = ethers.BigNumber.from(10).pow(18).mul('50000000')
  ...
  it("The Bonding NOM Contract should contain 50000000 NOM", async () => {
   let balance = await NOMtoken.balanceOf(BondingNOM.address)
   assert.equal(balance.toString(), "50000000000000000000000000");
  });
  ...
  it("should give ETH representing a range of NOM supply", async () => {
    let supplyTop = Math.random()*50000000;
  ...
  it("should load NOM onto bonding contract", async () => {
    let numTokens = ethers.utils.parseEther("50000000")
